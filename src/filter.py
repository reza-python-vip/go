...
def _create_remark(metrics: NodeMetrics, score: float) -> str:
    ...
    thr_display = (
        f"{thr_mbps:.2f}MB/s"
        if thr_mbps >= 1
        else f"{int(metrics.throughput_kbps)}KB/s"
    )
...
def filter_and_rank(
    ...
    reliable_ids = {
        nid
        for nid in successful_ids
        if history.get_reliability(nid) >= config.MIN_RELIABILITY
    }
    logger.info(
        f"{len(reliable_ids)} nodes meet the reliability threshold ({config.MIN_RELIABILITY:%})."
    )

    # 3. Performance filter
    hq_ids = {
        nid
        for nid in reliable_ids
        if (
            metrics_map[nid].latency_ms < config.MAX_LATENCY_MS
            and metrics_map[nid].throughput_kbps > config.MIN_THROUGHPUT_KBPS
        )
    }
    logger.info(
        f"{len(hq_ids)} nodes meet performance criteria (Latency < {config.MAX_LATENCY_MS}ms, Speed > {config.MIN_THROUGHPUT_KBPS}KB/s)."
    )
