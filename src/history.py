...
class NodeHistory:
    ...
    def add_result(self, success: bool):
        if (
            len(self.test_results) == self.test_results.maxlen
            and self.test_results[0] is False
        ):
    ...
    async def save_history(self):
        ...
        try:
            ...
            content = await loop.run_in_executor(
                None, lambda: json.dumps(data, indent=2)
            )
            await loop.run_in_executor(None, self.history_file.write_text, content)
            logger.info(
                f"Successfully saved history for {len(self.node_history)} nodes."
            )
        except IOError as e:
